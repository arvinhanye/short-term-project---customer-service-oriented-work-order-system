...
- 普通用户只能查看本人提交的工单详情。
- 管理员可查看所有工单详情。
- 新建工单先按分类、优先级匹配自动分配规则；无有效规则或无可用 ADMIN 时进入未分配池，认领后才能回复、备注和流转状态。
...
- 同一提交人对同一工单只能保留一条有效评价，重复提交必须被服务层拒绝。

### 3.4 SLA、通知与自动分配

- 系统按 LOW、MEDIUM、HIGH、URGENT 优先级应用首次响应、下次响应和解决时限，工作时间按工作日 09:00-18:00 计算。
- 管理员列表优先展示已超时和即将到期工单，在 SLA 到期前 60 分钟生成站内预警，超时后标记违约并升级通知 ROOT。
- 工单创建、认领、自动分配、转派、公开回复、催促、状态变化和 SLA 预警应生成带去重键的站内通知；用户可删除自己的单条通知或清空已读通知，不得删除其他账号的数据。
- 用户可选择所有回复、状态变更、仅处理结果或不通知；偏好使用结构化字段保存。
- 自动分配规则支持分类、优先级条件，以及指定 ADMIN 或最少待办两种动作；具体规则优先于兜底规则。
...
- 生命周期支持等待客户、暂挂、SLA 暂停/恢复、客户确认关闭和完成后 7 天内重新打开。
- 管理员可维护知识文章、快捷回复和处理宏；用户创建工单时可查看相关文章推荐。
- 服务质量报表包含首次响应、平均解决、SLA 达标率、积压、负载、一次解决率和满意度，并可导出 CSV/XLSX/PDF。
- ROOT 可执行日志归档、TTL、孤儿附件清理和备份恢复演练；所有登录会话支持空闲退出，敏感账号操作要求重新认证。
