...
flowchart TB
    ...
    service["Service 业务层"]
    dao["DAO 数据访问层"]
    mysql["MySQL: users / orders / SLA / rules / notifications / ratings / history"]
...
| `UserWorkbenchPanel` | 普通用户工作台，提供游标分页的我的工单、SLA 展示、通知角标、创建工单、个人资料维护和会话清理 |
| `AdminWorkbenchPanel` | 管理员中心，统一提供 SLA 风险队列、通知中心、自动分配规则、工单、用户、分类、统计、日志、自检、连接池监控和批量维护入口 |
| `NotificationDialog` | 异步读取最近站内通知，区分已读/未读并支持全部标记已读 |
...
| `SlaService` | 匹配优先级策略，按工作日 09:00-18:00 计算首次、下次响应和解决截止时间 |
| `NotificationService` | 按用户偏好生成、去重、已读站内通知，补扫 SLA 预警并将超时升级到 ROOT |
| `AssignmentRuleService` | 校验和管理分类/优先级条件及指定管理员/最少待办动作 |
| `ServiceManagementSchemaService` | 启动时校验 SLA、通知、分配规则和唯一评价所需数据库结构 |
...
| 数据源 | DAO |
| --- | --- |
| MySQL | `UserDAO`、`ProfileDAO`、`CategoryDAO`、`ItemDAO`、`OrderDAO`、`SlaPolicyDAO`、`AssignmentRuleDAO`、`NotificationDAO`、`TicketRatingDAO`、`TicketHistoryDAO`、`SystemLogImportDAO` |
...
| `profiles` | 用户资料和结构化通知偏好，与用户一对一 |
| `categories` | 工单分类，业务层严格限制为一级、二级结构 |
| `items` | 工单主记录，保存标题、分类和状态 |
| `orders` | 工单当前工作流：状态、负责人、待确认转派、催促、SLA 截止时间和乐观锁版本 |
| `sla_policies` | 按优先级配置首次/下次响应和解决时限 |
| `ticket_assignment_rules` | 自动分配的条件、动作、排序和启用状态 |
| `notifications` | 带去重键和已读时间的站内通知 |
| `ticket_ratings` | 评价唯一登记，与 MongoDB 评价正文共用事件编号 |
...
3. MySQL 事务中匹配 SLA 策略和自动分配规则，写入 `items`、`orders`、创建/自动分配历史及站内通知。
...
| 用户评分 | 工单提交人 | MongoDB `comments` + MySQL `ticket_ratings` 唯一登记 |
...
新建工单先按规则具体度、排序值匹配负责人；指定管理员不可用时继续尝试后续规则，没有可用结果才保持 `assigned_admin_id` 为空。ADMIN 认领使用 MySQL 行锁与 `workflow_version` 条件更新，只有仍为未分配且无待处理邀请时才能成功。转派保存唯一请求 ID、发起人、目标管理员、原因和时间，目标管理员接受时才替换负责人；拒绝、撤销和旧请求重复提交均不会覆盖新状态。每次版本变化必须同步插入相同 `event_seq` 的历史，任一步失败则整个 MySQL 事务回滚。

普通用户读取时间线时 SQL 只返回 `PUBLIC`，服务层再移除管理员身份、内部原因和 JSON 载荷；ADMIN 可以查看包含转派原因、内部备注事件和操作者的团队协作时间线。

### 5.5 SLA、通知与升级

`BusinessTimeUtil` 跳过周末和非 09:00-18:00 时段计算截止时间。首次管理员回复会固化首响时间；客户后续回复会重建下次响应时钟，管理员回复后清除该时钟；完成或关闭后将 SLA 标记为达标或超时。

工作台每 60 秒异步刷新通知未读数。`NotificationService` 查询一小时内的下一截止时间，通过去重键补发预警；超时后原子标记 `BREACHED`，通知当前负责人并升级到启用状态的 ROOT。ROOT 刷新通知时会全局补扫，避免只依赖某个负责人客户端。
...
系统定义 `BusinessException` 表示可提示给用户的业务错误，`DBException` 表示数据库访问错误。创建工单涉及 MySQL 和 MongoDB 两类数据源，采用“MySQL 事务 + MongoDB 补偿删除”的方式降低跨库不一致风险；若补偿删除暂时失败，会写入 MySQL `cross_db_repair_records`，下次启动时重试。评价在插入 MongoDB 后会于同一 MySQL 事务中预留 `(item_id,user_id)` 唯一登记，冲突时删除本次 MongoDB 事件，从而防止并发双评。MongoDB 行为日志或系统日志写入失败时，日志会写入 MySQL `pending_mongo_writes`，下次启动时重放到 MongoDB。批量状态维护通过 MySQL 存储过程执行，并记录系统审计日志。
...
| SLA 风险队列 | `orders(sla_state, first_response_due_at, next_response_due_at, resolution_due_at)` |
| 通知未读数 | `notifications(user_id, read_at, created_at)` |
...
| `BusinessTimeUtilTest` | 工作日起点归一、跨周末和自然分钟计算 |
| `SlaDisplayUtilTest` | 识别已迟到的首次响应时钟，避免被更晚的解决时限掩盖 |
| `ServiceManagementDAOTest` | 规则具体度、最少待办、通知去重/已读/升级对象和评价唯一性 |
...
2. 新库按 README 顺序执行 MySQL 与 MongoDB 初始化脚本；旧库额外执行可重复运行的 `mysql_service_management.sql`。
...
| 报表导出 | 已实现 CSV、XLSX、PDF；后续可增加定时报表投递 |
| 日志归档 | 已实现在线日志归档与归档集合 TTL；后续可接对象存储冷归档 |

## 12. P1 设计补充

- 状态 5/6 分别表示等待客户与暂挂，`sla_paused_at` 和累计暂停分钟负责顺延时钟；原虚拟转派待确认展示码迁移为 99。
- `knowledge_articles`、`reply_templates`、`handling_macros` 分离内容、模板与组合动作，避免把宏固化在 UI。
- 报表导出使用 Apache POI 生成 XLSX、PDFBox 生成 PDF；CSV 带 UTF-8 BOM，便于 Excel 直接打开中文。
- 日志先归档再从在线集合删除，归档集合按 `archived_at` 设置一年 TTL；清理 GridFS 前从评论附件字段收集引用集合。
