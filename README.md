...
- UI 按 P0-P3 完成改造：统一视觉规范，减少空白和分散入口；管理员工作概览提供个人状态环形图、待办风险列表和“我的工单”快捷筛选，收到的转派会以虚拟展示状态 99“待确认”进入指标、环形图和风险首位，点击可直接处理；创建工单采用紧凑双栏布局；管理员用户管理支持筛选、启用与禁用账号；主要弹窗支持 Esc 关闭。
...
- 新建工单先按分类和优先级匹配自动分配规则；无有效规则或无可用 ADMIN 时进入“未分配池”。ADMIN 可确认认领，或向其他 ADMIN 发起接手邀请。跨管理员转派必须填写原因并经目标管理员接受，在接受前原负责人不变。
...
- USER 与 ADMIN 的工单沟通支持发送文字、行内表情、文件和图片；表情从消息工具栏插入当前光标位置，附件既可点击选择也可直接拖放。附件使用 MongoDB GridFS 共享存储，单条消息最多 5 个、单文件 10 MB、合计 25 MB。图片可预览，其他文件可打开或另存，下载时会再次校验工单归属和内部备注可见性。
...
- 新建工单按分类、优先级匹配自动分配规则，可指定 ADMIN 或选择当前待办最少的 ADMIN；管理员可在“分配规则”中新增、启用和停用规则。
- 工单按优先级套用 SLA 策略，工作时间统一为工作日 09:00-18:00；用户与管理员列表、详情显示首次响应、下次响应和解决倒计时，管理员队列将已超时和即将到期工单优先展示。
- USER 与 ADMIN/ROOT 顶部新增站内通知中心，覆盖创建、认领、自动分配、转派、回复、催促、状态和 SLA 预警；前台每分钟刷新未读数，SLA 超时同步升级给 ROOT。通知使用去重键避免重复生成，用户通知偏好改为结构化字段。
- 每张工单每个提交人只允许一条有效评价；服务层通过 MongoDB 历史检查和 MySQL 唯一登记双重防止并发重复评分。
- 工单增加“等待客户回复”和“暂挂”状态，进入后暂停 SLA；恢复处理或客户回复时顺延截止时间。管理员完成后保留 7 天重开窗口，客户可确认关闭或填写原因重新打开。
- 知识库、快捷回复和处理宏已接入创建与处理工作台；内置 20 篇真实知识文章、20 条标准回复和 20 个处理宏，创建工单时可推荐文章，管理员可一键执行标准处理。
- 服务质量报表新增首次响应、解决时间、SLA 达标率、积压时长、管理员负载、一次解决率和满意度趋势，并支持 UTF-8 CSV、XLSX 与 PDF 导出。
- ROOT 数据生命周期面板支持 MongoDB 日志归档、归档集合 TTL、孤儿 GridFS 清理与 SQL 备份校验；`scripts/restore-drill.sh` 可在隔离临时库执行实际恢复演练。
- 所有账号治理敏感操作重新校验当前密码，登录会话连续 15 分钟无操作自动退出。
...
旧版本数据库还必须执行 `src/main/resources/sql/mysql_service_management.sql`。该脚本可重复执行，用于增加 SLA 策略与截止时间、站内通知、自动分配规则、结构化通知偏好和评价唯一登记；历史工单的 SLA 保持为空，不会因升级时间被误判为超时。应用启动时会校验这些结构，缺失时给出明确升级提示。

完成上述服务管理升级后，再执行 `src/main/resources/sql/mysql_p1_features.sql`。该增量脚本新增 SLA 暂停/重开字段、知识文章、快捷回复、处理宏和生命周期审计表，并清理已取消的保存视图与 MFA 遗留表；脚本可重复执行且不会删除现有工单。

旧版 MongoDB 同时执行一次 `src/main/resources/sql/mongodb_p1_lifecycle.js`，只创建日志归档与一年 TTL 索引，不会改写在线日志或工单数据。

旧版本 MongoDB 可执行一次 `src/main/resources/sql/mongodb_attachments.js`，为评论附件查询补充索引。脚本可安全重复执行；GridFS 的 `ticket_files` 集合会在首次上传时自动创建，不需要清空或迁移历史评论。
